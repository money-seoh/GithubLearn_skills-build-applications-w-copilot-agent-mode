...
export default function Workouts() {
  ...
  useEffect(() => { fetch(workoutsEndpoint).then((response) => { if (!response.ok) throw new Error(`Unable to load workouts (${response.status})`); return response.json() }).then((payload) => setWorkouts(normalizeCollection(payload))).catch((reason) => setError(reason.message)).finally(() => setLoading(false)) }, [])
