...
export default function Activities() {
  ...
  useEffect(() => { fetch(activitiesEndpoint).then((response) => { if (!response.ok) throw new Error(`Unable to load activities (${response.status})`); return response.json() }).then((payload) => setActivities(normalizeCollection(payload))).catch((reason) => setError(reason.message)).finally(() => setLoading(false)) }, [])
